...
public class Utils {
	...
	private static char[] CONVERTED = { '_', '\0', '\0', '\0', '\0', '\0',
	...
	public static String toUrlFriendly(String s) {
		int length = Math.min(s.length(), 236);
		// skip trailing slashes
		while (s.charAt(length - 1) == '/' || s.charAt(length - 1) == '\\') {
			length--;
		}
		int n = 0;
		...
		for (int i = 0; i < length; i++) {
			...
			if (index >= 0) {
				buffer[n++] = CONVERTED[index];
			} else {
				buffer[n++] = ch;
			}
			// skip not printable characters
			if (buffer[n-1] >= 0 && buffer[n-1] <= 31) {
				n--;
			}
		}
