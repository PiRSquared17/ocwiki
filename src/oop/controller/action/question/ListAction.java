...
public class ListAction extends AbstractAction {
	...
	public void performImpl() throws Exception {
		...
		if (getParams().hasParameter("topic")) {
			...
			questions = BaseQuestionDAO.fetchByTopic(id, page * PAGE_LENGTH, PAGE_LENGTH);
			Topic topic = TopicDAO.fetchById(id);
			title("Danh sách các câu hỏi thuộc chủ đề " + topic.getName() + " trên website trắc nghiệm CSForce");
			count = BaseQuestionDAO.countByTopic(id);
		} else if (getParams().hasParameter("author")) {
			long id = getParams().getLong("author");
			questions = BaseQuestionDAO.fetchByAuthor(id, page * PAGE_LENGTH, PAGE_LENGTH);
