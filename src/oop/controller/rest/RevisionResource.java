...
public class RevisionResource extends AbstractResource{
	...
	public ListResult<Revision<? extends Article>> listByResource(
			@PathParam("resourceID") long resourceID, 
			@DefaultValue("1") @QueryParam("page") int page,
			@DefaultValue("50") @QueryParam("size") int size){
		assertParamValid(size <= MAX_PAGE_SIZE, "size", "too large");
		List<Revision<? extends Article>> revList = RevisionDAO.fetchByResource(resourceID,(page-1)*size,size);
		String nextUrl = null;
		if (revList.size() >= size) {
			nextUrl = "/revisions?page=" + page + "&size=" + size;
		}		
		return new ListResult<Revision<? extends Article>>(revList, nextUrl);
	}	
}
