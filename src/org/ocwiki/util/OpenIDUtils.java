...
public final class OpenIDUtils {
	...
	public static String authRequest(String userSuppliedString,
			boolean connect, AbstractAction action, HttpSession session)
			throws IOException {
		...
		try {
			...
			String returnToUrl = ActionUtil
					.getActionURL("user.login.openid.verification");
			...
			if (userSuppliedString.startsWith(googleEndpoint)
					|| userSuppliedString.startsWith(googleEndpoint_s)) {
				...
				fetch.addAttribute("lastname",
					...
					|| userSuppliedString.startsWith(yahooEndpoint_s)) {
				fetch.addAttribute("email",
						"http://axschema.org/contact/email", true);
				...
				fetch.addAttribute("email_2",
						"http://axschema.org/contact/email", true);
				fetch.addAttribute("firstname_2",
						"http://axschema.org/namePerson/first", true);
				fetch.addAttribute("lastname_2",
						"http://axschema.org/namePerson/last", true);
				fetch.addAttribute("fullname_2",
						"http://axschema.org/namePerson", true);
			}
			...
			if (action == null) {
				...
				/*
				 * return null; } else { // Option 2: HTML FORM Redirection
				 ...
				 * .getDestinationUrl(false)); dispatcher.forward(httpReq,
				 * httpResp); }
				 */
			}
	...
	public static Identifier verifyResponse(boolean connect,
			AbstractAction action, HttpSession session)
		...
		try {
			...
			if ((session.getAttribute("OIDManager") != null)
					&& (session.getAttribute("openid-disc") != null)) {
				...
				if (connect == true) {
					...
					throw new ActionException(
							"Đăng nhập bằng openID thất bại: Lỗi phiên làm việc. Bạn hãy thực hiện lại.");
				}
			...
			if (verified != null) {
				...
				String providerUrl = (String) session
						.getAttribute("providerUrl");
				...
				if (found != null) {
					...
					if (authSuccess.hasExtension(AxMessage.OPENID_NS_AX)) {
						...
						List fullNames = fetchResp
								.getAttributeValues("fullname_2");
						if (fullNames.size() > 0) {
							if (!isEmpty((String) fullNames.get(0)))
								newUser.setFirstName((String) fullNames.get(0));
						}

						List firstNames = fetchResp
								.getAttributeValues("firstname_2");
						if (firstNames.size() > 0) {
							if (!isEmpty((String) firstNames.get(0)))
								newUser
										.setFirstName((String) firstNames
												.get(0));
						}
						List lastNames = fetchResp
								.getAttributeValues("lastname_2");
						if (lastNames.size() > 0) {
					...
					if (isEmpty(newUser.getFullname()))
						newUser.setFirstName("người dùng" + newUser.getId());

					session.setAttribute("newOIDAcc", new OpenIDAccount(
							userUrl, providerUrl, newUser));
					session.setAttribute("newUser", newUser);

					// OpenIDAccount newOpenIDAccount = new
					// OpenIDAccount(userUrl, newUser);
