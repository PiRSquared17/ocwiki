...
<div class="session">
    <div class="jsmenu" id="user-toolbar" style="display: ${sessionScope.login ? 'block' : 'none'};">
        <ul class="level1 horizontal" id="user-toolbar-root">
           <li class="level1">
               ${sessionScope.user.fullname}
               <ul class="level2 dropdown">
        <li><ocw:actionLink name="user.edituser" title="Sửa thông tin người dùng">Sửa thông tin</ocw:actionLink></li>
        <li><ocw:actionLink name="user.preference">Tuỳ chọn</ocw:actionLink></li>
        <li><ocw:actionLink name="user.logout" title="Đăng xuất khỏi hệ thống">Đăng xuất</ocw:actionLink></li>
               </ul>
           </li>
        </ul>
    </div>
    <div style="display: ${sessionScope.login ? 'none' : 'block'};" id="guest-toolbar">
	    <ul>
		     <li><ocw:actionLink name="user.signup">Đăng kí</ocw:actionLink></li>
		     <li><ocw:actionLink name="user.login" title="Đăng nhập" onclick="openLoginDialog(); return false;">Đăng nhập</ocw:actionLink></li>
	    </ul>
    </div>
</div>
...
<div style="display: none" id="session-loginDialog-content">
    <form>
		<p>
	   	<label>Tên:
       		<input type="text" name="name" id="session_name"/>
		</label>
        <span class="error-validating" id="session_nameError"></span>
		</p>
		<p>
		  <label>Mật khẩu:
		  <input type="password" name="password" id="session_password" />
		  <span class="error-validating" id="session_passwordError"></span>
		  </label>
		 </p>
    </form>
...
<!--
var userToolbar = new Menu('user-toolbar-root', 'userToolbar', 
        function () { this.closeDelayTime = 300; } );
...
function session_login() {
	...
	new Ajax.Request(restPath + '/login', {
        method: 'post',
        parameters: {
		   name: $F('session_name'),
