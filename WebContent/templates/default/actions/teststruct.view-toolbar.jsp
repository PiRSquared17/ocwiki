...
<div class="toolbar">
	<c:if test="${sessionScope.login && sessionScope.user.group=='teacher'}">
		<button onclick="location.href='${scriptPath}?action=sectstruct.create&tstr=${teststruct.id}'">Thêm mục</button>
		...
		<button onclick="location.href='${scriptPath}?action=test.create&usestruct=true&structname=${teststruct.name}&struct=${teststruct.id}'">Tạo đề</button>
